...
/* Post-ride modal JS */
'var _pmInited=false;',
'function openPostModal(){var o=document.getElementById("pmOverlay");if(!o)return;o.classList.add("open");document.body.style.overflow="hidden";if(!_pmInited){_pmInited=true;var anon=document.getElementById("pm-anon");var form=document.getElementById("pm-form");var nudge=document.getElementById("pm-phone-nudge");if(_userTier===0){anon.style.display="block";form.style.display="none";}else{anon.style.display="none";form.style.display="block";var ni=document.getElementById("pm-name");if(ni&&_userName&&!ni.value)ni.value=_userName;var pi=document.getElementById("pm-phone");if(pi){if(_userTier>=2&&_userPhone){pi.value="+"+_userPhone;pi.readOnly=true;pi.style.background="#f5f5f7";if(nudge)nudge.style.display="none";}else{if(nudge)nudge.style.display="block";}}var di=document.getElementById("pm-date");if(di)di.min=new Date().toISOString().split("T")[0];}}}',
