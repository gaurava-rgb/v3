...
function PAGE_HTML(subtitle, toPills, fromPills, cityOptions, dateBlocksHtml, totalCount, groupCount, tier, userEmail, userPhone, tzPref, userDisplayName) {
    ...
    return '<!DOCTYPE html>\n<html lang="en">\n<head>\n' +
        ...
        '<div id="pm-verify-cta" style="display:none;">' +
          ...
          '<a id="pm-verify-btn" href="/verify/wa?returnTo=/" target="_blank" onclick="pmStartPolling()" class="pm-btn-primary" style="display:block;text-align:center;text-decoration:none;background:#25D366;">&#128242; Verify your number to post this ride</a>' +
        '</div>' +
        '<div id="pm-verified-flash" style="display:none;text-align:center;padding:12px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;font-size:14px;font-weight:600;color:#15803d;margin-bottom:10px;">&#10003; Verified! Fill in your details and post.</div>' +
...
/* Post-ride modal JS */
'var _pmInited=false;',
'function openPostModal(){var o=document.getElementById("pmOverlay");if(!o)return;o.classList.add("open");document.body.style.overflow="hidden";if(!_pmInited){_pmInited=true;var anon=document.getElementById("pm-anon");var form=document.getElementById("pm-form");var nudge=document.getElementById("pm-phone-nudge");var verifyCta=document.getElementById("pm-verify-cta");var submitBtn=document.getElementById("pm-submit");if(_userTier===0){anon.style.display="block";form.style.display="none";}else{anon.style.display="none";form.style.display="block";var ni=document.getElementById("pm-name");if(ni&&_userName&&!ni.value)ni.value=_userName;var pi=document.getElementById("pm-phone");if(pi){if(_userTier>=2&&_userPhone){pi.value="+"+_userPhone;pi.readOnly=true;pi.style.background="#f5f5f7";if(nudge)nudge.style.display="none";}else{if(nudge)nudge.style.display="block";}}if(_userTier<2){if(verifyCta)verifyCta.style.display="block";if(submitBtn){submitBtn.disabled=true;submitBtn.style.opacity="0.4";submitBtn.title="Verify WhatsApp first";}}var di=document.getElementById("pm-date");if(di)di.min=new Date().toISOString().split("T")[0];}}}',
