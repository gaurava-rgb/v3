...
function displayName(req, isLoggedIn) {
    var name = req.sender_name && req.sender_name !== 'Unknown' ? req.sender_name : null;
    if (isLoggedIn) {
        return name || req.source_contact || 'Someone';
    }
    if (name) return redactName(name);
    return redactContact(req.source_contact) || 'Someone';
}
...
function buildClusters(requests, strict) {
    ...
    for (var ri = 0; ri < routes.length; ri++) {
        ...
        for (var gvi = 0; gvi < groupValues.length; gvi++) {
            ...
            // Deduplicate by source_contact: keep most recent row per contact.
            var seenContacts = {};
            var deduped = [];
            for (var di = 0; di < g.length; di++) {
                var contact = g[di].source_contact || ('__nocontact__' + di);
                var prev = seenContacts[contact];
                if (!prev || g[di].created_at > prev.created_at) {
                    seenContacts[contact] = g[di];
                }
            }
            var contactKeys = Object.keys(seenContacts);
            for (var ci = 0; ci < contactKeys.length; ci++) deduped.push(seenContacts[contactKeys[ci]]);
            g = deduped;
