...
function renderHousingBoard(listings, activeFilter, isLoggedIn) {
    ...
    function buildCard(l) {
        ...
        var snippetHtml = '';
        if (l.message_text) {
            snippetHtml = '<div class="snippet-row">&ldquo;' + h.escHtml(l.message_text.slice(0, 200)) + '&rdquo;</div>';
        }

        // Contact info or auth gate
        var contactHtml = '';
        if (isLoggedIn) {
            // Show contact info for logged-in users
            var resolvedPhone = l.poster_phone || l.contact_phone || null;
            var phoneDisplay = '';
            if (resolvedPhone) {
                var cp = String(resolvedPhone).replace(/\D/g, '');
                // Strip leading 1 (US country code) if present
                if (cp.length === 11 && cp[0] === '1') cp = cp.slice(1);
                phoneDisplay = cp.length === 10
                    ? '(' + cp.slice(0,3) + ') ' + cp.slice(3,6) + '-' + cp.slice(6)
                    : resolvedPhone;
            }
            // Only show contact_info if it's not just a duplicate of the phone number
            var showContactInfo = l.contact_info && (!resolvedPhone || l.contact_info.replace(/\D/g, '') !== String(resolvedPhone).replace(/\D/g, ''));
            if (resolvedPhone || showContactInfo) {
                contactHtml = [
                    '<div class="detail-row"><span class="dr-label">Contact</span><span class="dr-val">',
                    resolvedPhone ? '<strong>Phone:</strong> ' + h.escHtml(phoneDisplay) : '',
                    (resolvedPhone && showContactInfo) ? '<br>' : '',
                    showContactInfo ? h.escHtml(l.contact_info) : '',
                    '</span></div>'
                ].join('');
            }
        } else {
            // Show auth gate for logged-out users
            contactHtml = [
                '<div class="card-auth">',
                '  <div class="card-auth-title">&#128274; Verify to see contact info</div>',
                '  <div class="card-auth-body">Sign in with your @tamu.edu email and confirm your number via WhatsApp. Takes ~60 seconds.</div>',
        ...
        return [
            '<article class="listing ' + cls + '"' + city + ' tabindex="0" role="button" aria-expanded="false">',
    ...
    var authLinkHtml = isLoggedIn
        ? '<div class="auth-link"><a href="/profile">My Profile</a></div>'
        : '<div class="auth-link"><a href="/login">Sign in with @tamu.edu</a> to see poster contact info</div>';

    var authBannerHtml = isLoggedIn ? '' : [
        '<div class="auth-banner" id="auth-banner">',
        '  <span>&#128274; Contact info is hidden. <a href="/login">Sign in with your @tamu.edu email</a> to see it.</span>',
    ...
    var CSS = [
        ...
        '.detail-row { padding: 9px 14px; border-bottom: 1px solid #f0f0f0; font-size: 13px; display: flex; align-items: baseline; gap: 10px; }',
...
function renderListingPage(listing, isLoggedIn, isWaVerified) {
    ...
    if (isWaVerified) {
        ...
        contactHtml = [
            ...
            '  <div class="auth-gate-trust"><span class="check">&#10003;</span><span>One-time verification via WhatsApp OTP &mdash; takes about 60 seconds.</span></div>',
            '  <div class="auth-gate-actions">',
            '    <a class="btn-primary" href="/login/phone">Verify via WhatsApp</a>',
            '  </div>',
            '</div>'
        ].join('\n');
    } else {
        ...
        contactHtml = [
            '<div class="auth-gate">',
            '  <div class="auth-gate-icon">&#128242;</div>',
            '  <div class="auth-gate-title">Verify to see contact info</div>',
            '  <div class="auth-gate-body">This listing was posted by a real TAMU student in a private WhatsApp group. We verify phone numbers to keep the board spam-free and make sure the poster only hears from legit students.</div>',
