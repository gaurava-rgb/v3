...
function renderHousingBoard(listings, activeFilter, isLoggedIn) {
    ...
    if (!listings || listings.length === 0) {
        ...
        for (var i = 0; i < listings.length; i++) {
            ...
            var rt = relTime(l.created_at);
            var fc = freshClass(l.created_at);

            cardParts.push(
                '<a class="card-link" href="/listing/' + slug + '">' +
                '<div class="listing-card">' +
                (rt ? '  <div class="freshness"><span class="fresh-dot ' + fc + '"></span><span>' + rt + '</span></div>' : '') +
                ...
                (senderName ? '<span class="listing-sender">' + senderName + '</span>' : '<span></span>') +
                '    <span class="listing-source">' + WA_SVG + 'from WA group</span>' +
                '  </div>' +
                '</div>' +
                '</a>'
            );
    ...
    return [
        ...
        '  .board-subhead { font-size: 14px; color: #555; display: flex; align-items: center; gap: 6px; margin-bottom: 24px; }',
        '  .wa-dot { display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: #25d366; flex-shrink: 0; }',
        ...
        '                  padding: 18px 20px; transition: box-shadow 0.15s, transform 0.15s; position: relative; }',
        '  .listing-card:hover { box-shadow: 0 4px 16px rgba(0,0,0,0.10); transform: translateY(-2px); }',
        '  .freshness { position: absolute; top: 16px; right: 16px; display: flex; align-items: center;',
        '               gap: 4px; font-size: 11px; color: #888; }',
        '  .fresh-dot { width: 7px; height: 7px; border-radius: 50%; flex-shrink: 0; }',
        '  .fresh-dot.green { background: #22c55e; }',
        '  .fresh-dot.yellow { background: #f59e0b; }',
        '  .fresh-dot.gray { background: #d1d5db; }',
        ...
        '  .listing-location { font-size: 16px; font-weight: 700; margin-bottom: 6px; padding-right: 60px; }',
        ...
        '  .listing-avail { font-size: 13px; color: #555; margin-bottom: 8px; }',
        ...
        '  .listing-source { font-size: 11px; color: #25d366; font-weight: 600;',
        ...
        '<div class="page">',
        '  <h1>Housing Board</h1>',
        '  <div class="board-subhead"><span class="wa-dot"></span>Listings from TAMU student WhatsApp groups &middot; Updated daily</div>',
        '  <div class="filters">',
        '    ' + filterChips,
        '  </div>',
        cardsHtml,
...
function renderListingPage(listing, isLoggedIn, isWaVerified) {
    ...
    if (isWaVerified) {
        ...
        contactHtml = [
            ...
            '    <a class="btn-secondary" href="/login">Already have an account? Sign in</a>',
            '  </div>',
