...
async function fetchUserListings(opts) {
    ...
    var results = await Promise.all([
        writeClient.from('v3_requests')
            .select('id, source_contact, sender_name, request_type, request_category, ride_plan_date, ride_plan_time, time_fuzzy, date_fuzzy, request_origin, request_destination, raw_message, created_at')
            .in('source_contact', phones)
            .order('created_at', { ascending: false }),
        writeClient.from('v3_housing')
