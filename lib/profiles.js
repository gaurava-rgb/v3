...
/**
 * Returns all phones linked to an email in user_profiles.
 */
async function getPhonesForEmail(email) {
    if (!email) return [];
    var { data, error } = await readClient
        .from('user_profiles')
    ...
    if (error) { console.error('[Profiles] getPhonesForEmail failed:', error.message); return []; }
    return (data || []).map(function(r) { return r.phone; }).filter(Boolean);
}

/**
 * Returns tier label for a raw phone number.
 *   T2 = phone in user_profiles AND email_verified (logged in via site + verified WA)
 *   T1 = phone in user_profiles but no verified email (rare — WA-only profile)
 *   T0 = no profile row at all (unknown user)
 */
async function tierForPhone(phone) {
    if (!phone) return 'T0';
    ...
    var { data } = await readClient
        ...
        .maybeSingle();
    if (!data) return 'T0';
    if (data.email && data.email_verified) return 'T2';
    return 'T1';
}

module.exports = { upsertProfile, getProfile, updateProfileName, linkEmailToProfile, isEmailWaVerified, getPhoneForEmail, getPhonesForEmail, tierForPhone };
