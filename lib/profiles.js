...
async function linkEmailToProfile(phone, email) {
    ...
    var { error } = await writeClient
        .from('user_profiles')
        .update({ email: email, email_verified: true, updated_at: new Date().toISOString() })
        .eq('phone', digits);
...
module.exports = { upsertProfile, getProfile, updateProfileName, linkEmailToProfile, isEmailWaVerified };
